#!/usr/bin/env python3
"""
Cost-optimized scraper for HU Berlin Jewish Businesses Database - Batch 1 (Pages 11-50)
Uses the same successful pattern-based extraction approach as pages 1-10.
"""
...
class Batch1BusinessScraper:
    """Cost-optimized scraper for pages 11-50 using successful pattern-based approach"""
...
def main():
    """Main scraping function for pages 11-50"""
    scraper = Batch1BusinessScraper()
    
    base_url = "https://www2.hu-berlin.de/djgb/public/en/find?q=&fq=&sort=unternehmenasc"
    pages_to_scrape = list(range(11, 51))  # Pages 11-50 (40 pages total)
    
    logger.info("Starting Jewish businesses database scrape for pages 11-50...")
    ...
    if all_businesses:
        # Save to JSON
        output_file = "/Users/samfrons/Desktop/storymaps/jewish_businesses_batch_1.json"
        scraper.save_to_json(all_businesses, output_file, "11-50")
        ...
        logger.info("=== BATCH 1 SCRAPING SUMMARY ===")
        logger.info(f"Total businesses extracted: {len(all_businesses)}")
        logger.info(f"Expected businesses (approx.): ~400 (10 per page × 40 pages)")
        logger.info(f"Extraction rate: {len(all_businesses)/400*100:.1f}%")
