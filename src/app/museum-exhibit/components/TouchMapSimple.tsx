...
const generateBusinesses = (totalCount: number = 10021) => {
  const districts = [
    { name: 'Mitte', lat: 52.52, lng: 13.405, density: 0.25 },
    { name: 'Charlottenburg', lat: 52.5165, lng: 13.3042, density: 0.2 },
    { name: 'Prenzlauer Berg', lat: 52.5384, lng: 13.4247, density: 0.15 },
    { name: 'Schöneberg', lat: 52.4839, lng: 13.3547, density: 0.1 },
    { name: 'Kreuzberg', lat: 52.499, lng: 13.4041, density: 0.1 },
    { name: 'Friedrichshain', lat: 52.5158, lng: 13.4546, density: 0.08 },
    { name: 'Wilmersdorf', lat: 52.487, lng: 13.315, density: 0.07 },
  ...
  for (const district of districts) {
    ...
    for (let i = 0; i < districtCount; i++) {
      ...
      businesses.push({
        ...
        type: ['Retail', 'Manufacturing', 'Services', 'Banking', 'Textiles'][
...
const TouchMapSimple: React.FC<TouchMapProps> = ({
  ...
  useEffect(() => {
    const stats = {
      ...
      declining: visibleBusinesses.filter(
        (b) => b.status === 'declining' || b.status === 'pressure'
      ).length,
      takenOver: visibleBusinesses.filter((b) => b.status === 'takenOver').length,
      liquidated: visibleBusinesses.filter((b) => b.status === 'closed' || b.status === 'closing')
        .length,
    }
    onStatsUpdate?.(stats)
        ...
        roadLayers.forEach((layer) => {
          if (map.getLayer(layer)) {
            ...
            map.setPaintProperty(
              layer,
              'line-width',
              layer.includes('motorway') || layer.includes('trunk') ? 8 : 4
            )
          }
        })
  ...
  const getMarkerColor = (status: string) => {
    switch (status) {
      case 'active':
        return '#00D9D9' // Bright teal
      case 'pressure':
        return '#FFD93D' // Bright yellow
      case 'declining':
        return '#FFD93D' // Bright yellow
      case 'takenOver':
        return '#FF6B35' // Bright orange
      case 'closing':
        return '#FF6B35' // Bright orange
      case 'closed':
        return '#C589E8' // Bright purple
      case 'future':
  ...
  return (
    <div className="relative w-full h-full">
                ...
                {isHovered && (
                  ...
                  >
                    ...
                    {business.status === 'closed' && (
                      <div className="font-bold" style={{ color: '#C589E8' }}>
                        Closed {business.closedYear}
                      </div>
                    )}
                  </div>
                )}
          ...
          (() => {
            ...
            return (
              ...
              >
                <div
                  className="p-6"
                  style={{ backgroundColor: '#FFFFFF', border: '4px solid #000000' }}
                >
                  <h3 className="text-xl font-black uppercase mb-3" style={{ color: '#000000' }}>
                    {business.type}
                  </h3>
                  <p className="text-lg font-bold mb-3" style={{ color: '#000000' }}>
                    {business.district}, Berlin
                  </p>
                  <div className="text-sm space-y-2">
                    <div className="font-bold">Established: {business.establishedYear}</div>
                    {business.status === 'closed' && (
                      <div className="font-bold" style={{ color: '#C589E8' }}>
                        Forced closure: {business.closedYear}
                      </div>
                    )}
                    {business.status === 'future' && (
                      <div className="font-bold" style={{ color: '#8B8B8B' }}>
                        Will be established in {business.establishedYear}
                      </div>
                    )}
                    <div className="mt-3 pt-3" style={{ borderTop: '4px solid #000000' }}>
                      ...
                      >
                        <span className="font-black uppercase" style={{ color: '#000000' }}>
                          ...
                          {business.status === 'active'
                            ? 'Operating'
                            : business.status === 'pressure'
                              ? 'Under Pressure'
                              : business.status === 'declining'
                                ? 'Declining'
                                : business.status === 'takenOver'
                                  ? 'Taken Over'
                                  : business.status === 'closing'
                                    ? 'Being Liquidated'
                                    : business.status === 'closed'
                                      ? 'Closed'
                                      : 'Not Yet Established'}
      ...
      <div
        className="absolute bottom-4 left-4 px-4 py-2 text-sm font-black uppercase"
        style={{
