#!/usr/bin/env python3
"""
Cost-optimized scraper for HU Berlin Jewish Businesses Database - Batch 2 (Pages 51-150)
Uses the same successful pattern-based extraction approach as batch 1.
"""
...
class Batch2BusinessScraper:
    """Cost-optimized scraper for pages 51-150 using successful pattern-based approach"""
...
def main():
    """Main scraping function for pages 51-150"""
    scraper = Batch2BusinessScraper()
    
    base_url = "https://www2.hu-berlin.de/djgb/public/en/find?q=&fq=&sort=unternehmenasc"
    pages_to_scrape = list(range(51, 151))  # Pages 51-150 (100 pages total)
    
    logger.info("Starting Jewish businesses database scrape for pages 51-150...")
    ...
    if all_businesses:
        # Save to JSON
        output_file = "/Users/samfrons/Desktop/storymaps/jewish_businesses_batch_2.json"
        scraper.save_to_json(all_businesses, output_file, "51-150")
        ...
        logger.info("=== BATCH 2 SCRAPING SUMMARY ===")
        logger.info(f"Total businesses extracted: {len(all_businesses)}")
        logger.info(f"Expected businesses (approx.): ~1000 (10 per page × 100 pages)")
        logger.info(f"Extraction rate: {len(all_businesses)/1000*100:.1f}%")
