  ...
  return (
    <div className="w-full h-full flex flex-col" style={{ backgroundColor: 'var(--background)' }}>
      ...
      <div ref={listRef} className="flex-1 overflow-y-auto p-4 space-y-3">
        <div
          className="text-xs font-mono mb-3 uppercase tracking-wide font-semibold"
          style={{ color: 'var(--accent-orange)' }}
        >
          {allFilteredStories.length} locations found{' '}
          {displayCount < allFilteredStories.length && `(showing ${displayCount})`}
        </div>
                    ...
                    {(() => {
                      ...
                      return (
                        <span
                          ...
                          style={{
                            backgroundColor:
                              story.id === activeStoryId ? 'rgba(0, 0, 0, 0.2)' : 'var(--card-bg)',
                            color:
                              story.id === activeStoryId
                                ? 'var(--story-text-color, currentColor)'
                                : 'var(--foreground)',
...
export default React.memo(StoryList, (prevProps, nextProps) => {
  ...
  if (prevProps.visibleStories.length !== nextProps.visibleStories.length) {
    return false
  }

  // Compare scalar props
  const scalarPropsEqual =
  ...
  if (!scalarPropsEqual) return false

  // For arrays, compare first and last IDs (covers most state changes)
  // This is O(1) instead of O(n) JSON.stringify
  const prevStories = prevProps.visibleStories
  const nextStories = nextProps.visibleStories

  if (prevStories.length > 0) {
    const firstMatch = prevStories[0].id === nextStories[0].id
    const lastMatch =
      prevStories[prevStories.length - 1].id === nextStories[nextStories.length - 1].id
