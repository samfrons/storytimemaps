#!/usr/bin/env python3
"""
Geocode the test dataset using free Nominatim service with existing cache
"""
...
def geocode_test_dataset():
    """Geocode the complete test dataset"""
    print("=== Geocoding Test Dataset ===")
    ...
    if not os.path.exists(TEST_DATA_FILE):
        print(f"Error: {TEST_DATA_FILE} not found. Run merge_full_dataset.py first.")
        return
    ...
    print(f"\nProcessing {len(to_geocode)} addresses in {total_batches} batches...")
    ...
    for batch_num in range(total_batches):
        ...
        end_idx = min(start_idx + batch_size, len(to_geocode))
        batch = to_geocode[start_idx:end_idx]
        
        print(f"\nBatch {batch_num + 1}/{total_batches} ({len(batch)} addresses)")
        
        for local_idx, (i, business, address) in enumerate(batch):
            global_idx = start_idx + local_idx + 1
            print(f"  [{global_idx}/{len(to_geocode)}] {address[:50]}...")
        ...
        if geocoded_count > 0:
            ...
            print(f"    💾 Progress saved (backup: {backup_file})")
    ...
    print(f"Success rate: {geocoded_count/len(to_geocode)*100:.1f}%")
    
    # Final cache statistics
    cursor.execute("SELECT COUNT(*) FROM geocoding_cache")
    final_cache_count = cursor.fetchone()[0]
    print(f"Cache entries after geocoding: {final_cache_count}")
    
    conn.close()
    
    print(f"\n✅ Test dataset geocoding complete!")
    print(f"📁 Updated file: {TEST_DATA_FILE}")
    print(f"🌍 Ready for testing at: /test-full-dataset")

if __name__ == "__main__":
    geocode_test_dataset()