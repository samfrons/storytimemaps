...
const TouchMap: React.FC<TouchMapProps> = ({
  ...
  const unclusteredPointLayer = {
    ...
    paint: {
      ...
      'circle-radius': ['interpolate', ['linear'], ['zoom'], 10, 2, 12, 4, 14, 8, 16, 12],
      'circle-stroke-width': 1,
      'circle-stroke-color': '#ffffff',
      'circle-stroke-opacity': 0.8,
      'circle-opacity': ['interpolate', ['linear'], ['zoom'], 10, 0.6, 14, 0.9],
    },
  }
          ...
          {hoveredCluster.cluster ? (
            ...
            <div className="text-white">
              ...
              <div className="text-xs mt-1" style={{ color: hoveredCluster.color }}>
                {hoveredCluster.status === 'active'
                  ? 'Active Business'
                  : hoveredCluster.status === 'declining'
                    ? 'Under Pressure'
                    : hoveredCluster.status === 'takenOver'
                      ? 'Taken Over'
                      : 'Liquidated'}
              </div>
            </div>
