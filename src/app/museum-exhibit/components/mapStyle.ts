...
type MapboxLayerWithId = mapboxgl.Layer & { id: string }

export const applyCustomStyleToMap = (map: mapboxgl.Map) => {
  try {
    ...
    const layers = style.layers as MapboxLayerWithId[]
    ...
    layers.forEach((layer) => {
      ...
      // Admin boundaries
        ...
        layer.type === 'line' &&
        (layerId.includes('admin') || layerId.includes('boundary'))
      ) {
