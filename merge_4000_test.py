...
def main():
    print("Adding 2,000 more businesses for 4,000 total test...")
    
    # Load current storymaps.json (should have 2,000 stories)
    with open('data/storymaps.json', 'r') as f:
        current_stories = json.load(f)
    
    # Load batch 3 data (2,500 businesses) - we used 500, have 2,000 remaining
    ...
    # Take 2,000 more from batch 3 (we used first 500, so take 500-2500)
    batch3_subset = batch3_data['businesses'][500:2500]  # Skip first 500 already used, take next 2,000
    
    businesses_to_add = batch3_subset
    print(f"Adding {len(businesses_to_add)} businesses from batch 3 (positions 500-2500)")
    ...
    # Backup current file
    backup_name = f"data/storymaps_backup_4000test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    print("✓ Updated storymaps.json with 4,000 total stories")
    ...
    print(f"  Ready to test 4,000-story performance!")
    print(f"  This is 8x the original 500 story test.")
