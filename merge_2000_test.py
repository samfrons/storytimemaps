...
def main():
    print("Adding 1,000 more businesses for 2,000 total test...")
    
    # Load current storymaps.json (should have 1,000 stories)
    with open('data/storymaps.json', 'r') as f:
        current_stories = json.load(f)
    
    # Load batch 2 data (1,000 businesses) - we'll use the remaining 500
    # Load batch 3 data (2,500 businesses) - we'll use 500 from here
    with open('jewish_businesses_batch_2.json', 'r') as f:
        batch2_data = json.load(f)
    
    with open('jewish_businesses_batch_3.json', 'r') as f:
    ...
    print(f"Batch 3 businesses available: {len(batch3_data['businesses'])}")
    
    # Take remaining 500 from batch 2 (we used first 500 already)
    # Plus 500 from batch 3 to get exactly 1,000 more
    remaining_batch2 = batch2_data['businesses'][500:]  # Skip first 500 already used
    batch3_subset = batch3_data['businesses'][:500]     # Take first 500 from batch 3
    
    businesses_to_add = remaining_batch2 + batch3_subset
    print(f"Adding {len(remaining_batch2)} remaining from batch 2 + {len(batch3_subset)} from batch 3 = {len(businesses_to_add)} total")
    ...
    print(f"  - Added: {len(new_stories)}")
    
    # Backup current file
    backup_name = f"data/storymaps_backup_2000test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    print("✓ Updated storymaps.json with 2,000 total stories")
    ...
    print(f"  Ready to test 2,000-story performance")
