...
def main():
    print("Creating 500-business test dataset...")
    
    # Load current storymaps.json (125 stories)
    with open('data/storymaps.json', 'r') as f:
        current_stories = json.load(f)
    
    # Load batch 1 data (400 businesses)
    with open('jewish_businesses_batch_1.json', 'r') as f:
        batch1_data = json.load(f)
    
    print(f"Current stories: {len(current_stories)}")
    print(f"Batch 1 businesses available: {len(batch1_data['businesses'])}")
    
    # Take exactly 375 businesses from batch 1 to reach 500 total
    businesses_to_add = batch1_data['businesses'][:375]
    print(f"Adding first {len(businesses_to_add)} businesses from batch 1")
    ...
    print(f"  - Original: {len(current_stories)}")
    print(f"  - Added from batch 1: {len(new_stories)}")
    
    # Backup current file
    backup_name = f"data/storymaps_backup_500test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    print("✓ Updated storymaps.json with 500 total stories")
    ...
    print(f"  Ready to test system performance with 500 entries")
