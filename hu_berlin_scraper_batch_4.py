#!/usr/bin/env python3
"""
Cost-optimized scraper for HU Berlin Jewish Businesses Database - Batch 4 (Pages 401-1013)
Final batch to complete the entire database extraction using the same successful pattern-based approach.
"""
...
class Batch4BusinessScraper:
    """Cost-optimized scraper for pages 401-1013 using successful pattern-based approach"""
...
def main():
    """Main scraping function for pages 401-1013"""
    scraper = Batch4BusinessScraper()
    
    base_url = "https://www2.hu-berlin.de/djgb/public/en/find?q=&fq=&sort=unternehmenasc"
    pages_to_scrape = list(range(401, 1014))  # Pages 401-1013 (613 pages total)
    
    logger.info("Starting Jewish businesses database scrape for pages 401-1013...")
    logger.info(f"Total pages to scrape: {len(pages_to_scrape)}")
    logger.info("This is the FINAL BATCH to complete the entire database extraction!")
    ...
    if all_businesses:
        # Save to JSON
        output_file = "/Users/samfrons/Desktop/storymaps/jewish_businesses_batch_4.json"
        scraper.save_to_json(all_businesses, output_file, "401-1013")
        ...
        logger.info("=== BATCH 4 SCRAPING SUMMARY ===")
        logger.info(f"Total businesses extracted: {len(all_businesses)}")
        logger.info(f"Expected businesses (approx.): ~6130 (10 per page × 613 pages)")
        logger.info(f"Extraction rate: {len(all_businesses)/6130*100:.1f}%")
        ...
        logger.info("\n=== COMPLETE DATABASE EXTRACTION SUMMARY ===")
        logger.info("Total expected businesses across all batches:")
        logger.info("  Batch 1 (pages 1-50): ~500 businesses")
        logger.info("  Batch 2 (pages 51-150): ~1000 businesses") 
        logger.info("  Batch 3 (pages 151-400): ~2500 businesses")
        logger.info("  Batch 4 (pages 401-1013): ~6130 businesses")
        logger.info(f"  TOTAL DATABASE: ~10,130 businesses")
        logger.info(f"This batch extracted: {len(all_businesses)} businesses")
        logger.info("Database extraction is now COMPLETE!")
