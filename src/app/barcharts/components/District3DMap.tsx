...
const District3DMap: React.FC<District3DMapProps> = ({ theme }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const businessData = geoData.features.map((feature: GeoJSONFeature) => ({
          ...feature.properties,
    ...
    map.current.addLayer(
      {
        ...
        paint: {
          ...
          'fill-opacity': ['interpolate', ['linear'], ['zoom'], 10, 0.3, 15, 0.1],
        },
      },
      'hexagon-layer'
    )
  ...
  const getHexagonVertices = (center: [number, number], size: number): [number, number][] => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      vertices.push([center[0] + size * Math.cos(angle), center[1] + size * Math.sin(angle)])
    }
  ...
  const addParticleLayer = () => {
    ...
    map.current.addLayer({
      ...
      source: 'particles',
      paint: {
        'circle-radius': ['interpolate', ['linear'], ['get', 'height'], 50, 3, 350, 8],
        'circle-color': [
          'interpolate',
          ['linear'],
          ['get', 'height'],
  ...
  if (!mounted || !theme) {
    return (
      <div
        className="relative w-full h-[500px] flex items-center justify-center border"
        style={{
    ...
    <div className="relative w-full h-[500px]">
      ...
      <div className="absolute top-4 left-4 p-4 rounded-lg backdrop-blur-md bg-black bg-opacity-30 border border-white border-opacity-20">
        ...
        <div className="text-white text-sm font-mono space-y-1">
          <div>
            {t('barcharts.3dMap.totalBusinesses')}:{' '}
            <span className="text-cyan-400">{stats.total}</span>
          </div>
          <div>
            {t('barcharts.3dMap.districts')}:{' '}
            <span className="text-cyan-400">{Object.keys(stats.byDistrict).length}</span>
          </div>
          <div>
            {t('barcharts.3dMap.topDistrict')}:{' '}
